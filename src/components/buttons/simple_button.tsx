...
export function SimpleButton({ typeBtn, children, className, loading, disabled, ...rest }: iProps) {
    const type = {
    ...
    return <button {...rest} type={rest.type} disabled={loading?.isLoading || disabled} className={twMerge('px-2.5 py-2 border rounded-md w-max ', type[typeBtn], className)}>
